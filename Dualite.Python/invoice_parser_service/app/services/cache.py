...
class RedisCache:
    ...
    # Invoice-specific cache methods
    async def cache_invoice_result(self, job_id: str, result: Dict[str, Any]) -> bool:
        """Cache the result of invoice processing"""
        try:
            key = f"invoice:result:{job_id}"
            return await self.set(key, result, 86400)  # Cache for 24 hours
        except Exception as e:
            logger.error(f"Error caching invoice result: {str(e)}")
            return False
            
    async def get_invoice_result(self, job_id: str) -> Optional[Dict[str, Any]]:
        """Get cached invoice processing result"""
        try:
            key = f"invoice:result:{job_id}"
            return await self.get(key)
        except Exception as e:
            logger.error(f"Error retrieving cached invoice result: {str(e)}")
            return None
