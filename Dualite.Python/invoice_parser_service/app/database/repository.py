...
import logging
from .models import Base, ProcessingJob, InvoiceExtraction
...
class InvoiceExtractionRepository(Repository[InvoiceExtraction]):
    """Repository for Invoice Extractions"""
    
    def __init__(self, db: Session):
        super().__init__(db, InvoiceExtraction)
    
    async def get_by_job_id(self, job_id: Union[str, UUID]) -> Optional[InvoiceExtraction]:
        """Get invoice extraction by job ID"""
        try:
            return self.db.query(self.model_class).filter(self.model_class.job_id == job_id).first()
        except Exception as e:
            logger.error(f"Error getting invoice extraction for job {job_id}: {str(e)}")
            return None
            
    async def get_by_invoice_number(self, invoice_number: str) -> List[InvoiceExtraction]:
        """Get invoice extractions by invoice number"""
        try:
            return self.db.query(self.model_class).filter(self.model_class.invoice_number == invoice_number).all()
        except Exception as e:
            logger.error(f"Error getting invoice extractions for invoice number {invoice_number}: {str(e)}")
            return []
    
    async def update_validation_status(self, id: Union[str, UUID], status: str, messages: List[str]) -> Optional[InvoiceExtraction]:
        """Update validation status for an invoice extraction"""
        try:
            return await self.update(id, {"validation_status": status, "validation_messages": messages})
        except Exception as e:
            logger.error(f"Error updating validation status for invoice extraction {id}: {str(e)}")
            return None
